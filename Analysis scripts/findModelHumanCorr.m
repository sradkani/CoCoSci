function [ModelHumanCorrelation_each, ModelHumanCorrelation_avg] = findModelHumanCorr(sequences, subjRand_human, modelParams, delta, alpha)
    ...
    CorrMatEachResp = corr([subjRand_human_unrolled, subjRand_model_repeated]);
    ModelHumanCorrelation_each = CorrMatEachResp(1,2);
    
    % take the average of human ratings for each sequence
    subjRand_human_mean = cellfun(@(x) mean(x), subjRand_human);
    
    CorrMatAvgResp = corr([subjRand_human_mean', subjRand_model]);
    ModelHumanCorrelation_avg = CorrMatAvgResp(1,2);
    
    display(alpha)
    display(delta)
    display(ModelHumanCorrelation_each)
    display(ModelHumanCorrelation_avg)
end
